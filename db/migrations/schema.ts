import { pgTable, foreignKey, unique, serial, uuid, integer, timestamp, varchar, text, doublePrecision, date, boolean } from "drizzle-orm/pg-core"
...
export const dosageDetail = pgTable("dosage_detail", {
	dosageDetailId: serial("dosage_detail_id").primaryKey().notNull(),
	uuid: uuid(),
	description: varchar(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
}, (table) => [
	unique("dosage_detail_uuid_key").on(table.uuid),
]);

export const doctorAssistant = pgTable("doctor_assistant", {
...
export const documentType = pgTable("document_type", {
	...
	unique("document_type_uuid_key").on(table.uuid),
]);
...
export const drug = pgTable("drug", {
	...
	unique("drug_uuid_key").on(table.uuid),
]);

export const frequencyDetail = pgTable("frequency_detail", {
	frequencyDetailId: serial("frequency_detail_id").primaryKey().notNull(),
	uuid: uuid(),
	description: varchar(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
}, (table) => [
	unique("frequency_detail_uuid_key").on(table.uuid),
]);
...
export const disease = pgTable("disease", {
	...
	unique("disease_uuid_key").on(table.uuid),
]);
...
export const insuranceCompany = pgTable("insurance_company", {
	...
	unique("insurance_company_uuid_key").on(table.uuid),
]);

export const labTestResult = pgTable("lab_test_result", {
...
export const imagingTestImage = pgTable("imaging_test_image", {
...
export const role = pgTable("role", {
	...
	unique("role_uuid_key").on(table.uuid),
]);

export const vitalSign = pgTable("vital_sign", {
...
export const imagingTest = pgTable("imaging_test", {
...
export const medicalRecordDisease = pgTable("medical_record_disease", {
...
export const patient = pgTable("patient", {
	...
	unique("patient_uuid_key").on(table.uuid),
]);
...
export const userInsuranceAffiliateCode = pgTable("user_insurance_affiliate_code", {
	userInsuranceAffiliateCodeId: serial("user_insurance_affiliate_code_id").primaryKey().notNull(),
	uuid: uuid(),
	appUserId: integer("app_user_id"),
	insuranceId: integer("insurance_id"),
